...
#
# https://github.com/PreferredAI/cornac/blob/master/cornac/models/bivaecf/recom_bivaecf.py
...
class BiVAECF(Recommender):
    ...
    * Quoc-Tuan Truong, Aghiles Salah, Hady W. Lauw. " Bilateral Variational Autoencoder for Collaborative Filtering."
    ...
    def fit(self, train_set, val_set=None):
        ...
        import torch
        from .ModifyBivae import BiVAE, learn
        ...
        if self.trainable:
            ...
            if not hasattr(self, "bivaecf"):
                ...
                self.bivae = BiVAE(
                    ...
                    user_decoder_structure=self.decoder_structure,
                    item_decoder_structure=self.decoder_structure,
                    act_fn=self.act_fn,
                    likelihood=self.likelihood,
                    cap_priors=self.cap_priors,
                    feature_dim=feature_dim,
                    batch_size=self.batch_size,
                    hidden_scaler=self.hidden_scaler
                ).to(self.device)
    ...
    def score(self, user_idx, item_idx=None):
        ...
        if item_idx is None:
            ...
            known_item_scores = (
                self.bivae.decode_user(theta_u, beta).cpu().detach().numpy().ravel()
            )
            ...
            pred = self.bivae.decode_user(theta_u, beta_i).cpu().detach().numpy().ravel()
    ...
    def gen_recommend(self, file_name, replace_model=None, TOP_K=50):
        num_user = self.train_set.num_users
        file_name = file_name
        ...
        for uid in range(1, num_user + 1):
            count += 1
            if count % int(num_user / 20) == 0:
                print("|", end="")
            try:
                recommendations, scores = self.rank(self.train_set.uid_map[uid])
                item_idx2id = list(self.train_set.item_ids)
            except:
                assert replace_model is not None, "replace model do not define"
                recommendations, scores = replace_model.rank(replace_model.train_set.uid_map[uid])
                item_idx2id = list(replace_model.train_set.item_ids)
            items = " ".join([str(int(item_idx2id[i])) for i in recommendations[:TOP_K]])
